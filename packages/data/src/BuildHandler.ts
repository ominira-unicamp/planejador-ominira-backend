...
import { PathSegment, pathSegmentToOpenApiPath } from "@pomi/api-core";
...
export function buildHandler<
    ...
    Q extends z.ZodType,
    B extends z.ZodType,
    I extends InputSchemaTypes<PT, Q, B>,
    O extends outputSchemaType
>(
    inputSchema: I,
    outputSchema: O,
    fn: (ctx: Context, input: z.infer<I>) => Promise<z.infer<O>>
) {
...
export function openApiArgsFromIO(io: IO) {
    ...
    if (io.input.shape.body) {
        request = request.body(
            io.input.shape.body,
            io.input.shape.body.meta()?.description || "Request body"
        );
    }
