...
const buildZodSchemas = (
    ...
    existsMany: z
        .array(z.number().int())
        .refine(async (vals) => await verifyExistsMany(prismaDelegate, vals), {
            message: manyMessage,
            params: { code: "REFERENCE_NOT_FOUND" }
        })
});
...
const buildZodIds = (prisma: PrismaClient) => ({
    ...
    studyPeriod: buildZodSchemas(
        prisma.studyPeriod,
        "Study period not found",
        "One or more study periods not found"
    ),
    professor: buildZodSchemas(
        prisma.professor,
        "Professor not found",
        "One or more professors not found"
    ),
    room: buildZodSchemas(
        prisma.room,
        "Room not found",
        "One or more rooms not found"
    ),
    class: buildZodSchemas(
        prisma.class,
        "Class not found",
        "One or more classes not found"
    ),
    classSchedule: buildZodSchemas(
...
type DelageteType = {
    ...
    findMany(args: { where: { id: { in: number[] } } }): Promise<unknown[]>;
};
